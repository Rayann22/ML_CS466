...
def train_one_fold(train_texts, train_labels, val_texts, val_labels, max_len, device, model_mode, w2v=None):
    ...
    for epoch in range(epochs):
        ...
        print("    Epoch %d | loss %.4f | fold_val_acc %.4f" % (epoch + 1, avg_loss, val_acc))
...
def main():
    ...
    model_mode = "rand"   # choose: "rand", "static", "non_static"
    w2v = None
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(texts, labels), start=1):
        ...
        print("Fold %d best acc: %.4f" % (fold_idx, fold_acc))
    ...
    for i, acc in enumerate(fold_accuracies, start=1):
        print("Fold %d: %.4f" % (i, acc))
    print("Mean accuracy: %.4f" % mean_acc)
    print("Std: %.4f" % std_acc)
